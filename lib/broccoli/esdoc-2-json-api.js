...
function hoistPods(modules) {
  ...
  for (let parentModulePath in moduleMap) {
    ...
    if (modules.length === 1) {
      modules[0].attributes.path = parentModulePath;
    }
  }
}
...
function hoistDefaults(modules) {
  ...
  for (let m in modules) {
    ...
    if (module.relationships.components.length >= 1) {
      let klass = module.relationships.components[0];

      if (klass.attributes.exportType === 'default') {
        parentModule.relationships.components.push(klass);
        module.relationships.components.pop();
      }
    }
  ...
  for (let m in modules) {
    let module = modules[m];
    if (
      module.relationships.components.length === 0
    ...
    for (let doc of docs) {
      if (isModule(doc)) {
        ...
        if (Component.detect(doc)) {
          ...
          module.relationships.components.push(klass);
